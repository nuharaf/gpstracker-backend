...
func main() {
	...
	if *ws_server {
		ws := ws.NewWebstream(pool, srv, ws.WebStreamConfig{MockToken: *ws_server_mock_login, ListenAddr: *ws_server_listen_addr})
		go ws.Run()
		wg.Add(1)
	}
	if *api_server {
		api := web.NewApi(pool, srv, &web.ApiConfig{ListenAddr: *api_server_listen_addr, CookieDomain: *api_server_cookie_domain})
		go api.Run()
		wg.Add(1)
	}

...
func (l LoginHandler) login(ctx context.Context, username, password string) (*session, *UserInfo, error) {
	...
	sqlStmt := `SELECT id,"password",init_done,suspended,session_length_sec,roles FROM "user" WHERE username = $1`
	...
	err := row.Scan(&id, &hashpwd, &init_done, &suspended, &sess_length, &roles)
	...
	if !suspended {
		...
		if err != nil {
			...
			sqlStmt := `INSERT INTO session (session_id,user_id,csrf_token,ws_token,init_password,valid_until) 
			VALUES($1,$2,$3,$4,now(),$5)`
			_, err := l.db.Exec(ctx, sqlStmt, session_id, id, csrf_token, ws_token, true,
...
func (l *LoginHandler) init_password(ctx context.Context, session_id string, req *ChangePwdRequest, w http.ResponseWriter) {
	...
	_, err = tx.Exec(ctx, `UPDATE "user" SET password = $1,init_done = true WHERE id = $2`,
